...
datos_comercio <- left_join(
  ...
  mutate(
    sector = sector,
    memberStateCode = memberStateCode,
    total_imports_neg = -total_kg.y,
    total_exports = total_kg.x,
...
datos_comercio |> 
  ...
  scale_fill_manual(values = c(
    "Importaciones" = "#d7191c",
    "Exportaciones" = "#2c7bb6"
  )) +
  theme_minimal() + # Tema limpio
  labs(
    title = "Balanza Comercial Agroalimentaria UE",
    subtitle = "Comparativa de Exportaciones (Derecha) vs Importaciones (Izquierda)",
    x = NULL, 
    y = "Quilogramos (kg)",
    fill = "Tipo de Flujo",
