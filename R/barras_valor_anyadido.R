...
df <- get_eurostat("aact_eaa01", time_format = "num", unit = "T") |>
  filter(
    ...
    indic_ag == "PROD_BP", # VA_pb
     # Selección de la serie de cultivos
    itm_newa == 10000
  ) |>
  mutate(
    pais = geo,
...
df |>
  ...
  geom_text(
    aes(label = number(value, scale = 0.001, accuracy = 1)), 
    vjust = -0.4
  ) +
  scale_y_continuous(
  ...
  labs(
    ...
    title = paste0("Los 10 países de la UE-27" ,
      " con mayor producción agrícola vegetal"
    ),
    subtitle = paste0("Valores Añadidos a precios básicos",
      " corrientes (millones de euros, 2023)"
    ),
    caption = paste0("Elaboración propia con R.",
      " Eurostat (aact_eaa01, P11 – Output of crop production)."
    )
  ) +
  theme_bw() +
  theme(
